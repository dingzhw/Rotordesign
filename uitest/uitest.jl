...
function uitest(x2ro::Rotor,judge=false)
    ...
    taper = x2ro.taper.v
    global θtw = x2ro.twist.v

    # ---
    global ch = Array{Float64}(NR,Nbe) #桨叶弦长
    global rb = Array{Float64}(NR,Nbe)  #桨叶叶素点分段径向长度值
    global dr = Array{Float64}(NR,Nbe)  #桨叶叶素分段长度值
    ...
    beta_lat = 0.0
    beta_lon = 0.0
    global θcp = zeros(Float64,NR)
    global θlat = zeros(Float64,NR)
    global θlon = zeros(Float64,NR)
    global θ0 = Array{Float64}(NR,Nbe)
    for k in 1:NR
      θcp[k] = θ7
      θlat[k] = thelat
      θlon[k] = thelon
