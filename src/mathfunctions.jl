@everywhere function maprepend(a,b,i,j,k,l::Int64=1)
  # 多维矢量数组的新插值方法
...
# 坐标系转换矩阵
@everywhere function systoro(m1,ψ) #固定坐标系转为旋转坐标系
  m2 = zeros(Float64,3)
  stor = [cos(ψ) sin(ψ) 0;
          -sin(ψ) cos(ψ) 0;
          0 0 1]
...
@everywhere function rotosys(m2,ψ) #旋转坐标系转为固定坐标系
  m1 = zeros(Float64,3)
...
@everywhere function rotobeta(mro,beta) # 旋转坐标系到挥舞坐标系
...
@everywhere function betatoro(mbeta,beta) # 挥舞坐标系到旋转坐标系
  mro = zeros(Float64,3)
...
@everywhere function betatobe(mbeta,θ) # 从挥舞坐标系转换到叶素坐标系
  mbe = zeros(Float64,3)
  betobe = [1 0 0;
            0 -cos(θ) -sin(θ);
            0 sin(θ)  -cos(θ)]
...
@everywhere function rotobe(mr,beta,θ) # 从旋转坐标系转化到叶素坐标系
  mbe = zeros(Float64,3)
...
@everywhere function betoro(mbe,beta,θ) # 从叶素坐标系转换到旋转坐标系
  mr = zeros(Float64,3)
...
# 矢量夹角函数
@everywhere function aoaang(vec_,x_=[1,0]) # 二维坐标系下，任意矢量与x轴夹角(-180deg~180deg)
